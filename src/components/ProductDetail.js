...
export default function ProductDetail({ id }) {
  ...
  const isFavorite = favorites.some(fav => fav.id === productDetail._id);
  const cartItem = cart.find(item => item.id === productDetail._id);
    ...
    <div className="relative min-h-screen w-full px-5 py-10 flex justify-center">
      ...
      <div className="w-[60%] max-w-6xl grid md:grid-cols-2 gap-10 items-center">
        ...
        <div className="flex flex-col justify-center gap-6 w-full relative">
          ...
          {/* precio */}
          <p className="text-xl font-semibold text-green-400">Precio: ${selectedVariant?.price || 0}</p>
              ...
              <div>
                ...
                >
                  {selectedVariant?.sizes?.map((size, i) => (
                    <option key={`${productDetail._id}-size-${i}`} value={size}>{size}</option>
                  ))}
                </select>
              </div>
              <div>
                ...
                >
                  {productDetail.variants.map((v, i) => (
                    <option key={`${productDetail._id}-variant-${i}`} value={v.color}>{v.color}</option>
                  ))}
                </select>
              </div>
              <div>
          ...
          {selectedVariant?.features?.length > 0 && (
            <ul className="list-disc list-inside text-white mt-2">
              {selectedVariant.features.map((feat, i) => (
                <li key={`${productDetail._id}-feat-${i}`}>{feat}</li>
              ))}
            </ul>
          )}
          ...
          <div className="flex items-center gap-2 mt-4">
            {!cartItem ? (
              ...
              <>
                <button
                  onClick={e => { e.preventDefault(); decrementCartItem(productDetail._id); }}
                  className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 cursor-pointer"
                >-</button>
                <span className="text-white font-semibold">{cartItem.quantity}</span>
                <button
                  onClick={e => { e.preventDefault(); incrementCartItem(productDetail._id); }}
                  className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 cursor-pointer"
                >+</button>
              </>
            )}
